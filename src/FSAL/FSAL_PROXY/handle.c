/*
 * Copyright (C) Stony Brook University 2014-2015
...
#define FSAL_PROXY_CONTEXT_COUNT 64

#define FSAL_PROXY_ATTR_EXPIRE_SECONDS 5
...
/* Used for getattr */
static struct attrmask2bit attrmask2bit_getattr[] = {
	{ ATTR_TYPE, FATTR4_TYPE },
	{ ATTR_SIZE, FATTR4_SIZE },
	{ ATTR_FSID, FATTR4_FSID },
	{ ATTR_ACL, FATTR4_ACL },
	{ ATTR_FILEID, FATTR4_FILEID },
	{ ATTR_MODE, FATTR4_MODE },
	{ ATTR_NUMLINKS, FATTR4_NUMLINKS },
	{ ATTR_OWNER, FATTR4_OWNER },
	{ ATTR_GROUP, FATTR4_OWNER_GROUP },
	{ ATTR_ATIME, FATTR4_TIME_ACCESS },
	{ ATTR_MTIME, FATTR4_TIME_MODIFY },
	{ ATTR_CTIME, FATTR4_TIME_METADATA },
	{ ATTR_CHANGE, FATTR4_CHANGE },
	{ ATTR_SPACEUSED, FATTR4_SPACE_USED },
};
...
static struct bitmap4 pxy_attrmask_to_bitmap(attrmask_t attrmask, bool is_setattr)
{
	int i;
	struct bitmap4 bmap = empty_bitmap;
	struct attrmask2bit *mapping;
	int mapping_len;

	if (attrmask == 0) {
		return pxy_bitmap_getattr;
	}

	if (is_setattr) {
		mapping = fsal_mask2bit;
		mapping_len = ARRAY_SIZE(fsal_mask2bit);
	} else {
		mapping = attrmask2bit_getattr;
		mapping_len = ARRAY_SIZE(attrmask2bit_getattr);
	}

	for (i = 0; i < mapping_len; i++) {
		if (FSAL_TEST_MASK(attrmask, mapping[i].mask)) {
			if (mapping[i].fattr_bit > 31) {
				bmap.map[1] |=
				    1U << (mapping[i].fattr_bit - 32);
				bmap.bitmap4_len = 2;
			} else {
				bmap.map[0] |= 1U << mapping[i].fattr_bit;
			}
		}
	}
...
static int pxy_fsalattr_to_fattr4(const struct attrlist *attrs, fattr4 *data)
{
	...
	bmap = pxy_attrmask_to_bitmap(attrs->mask, true);
...
static int pxy_rpc_read_reply(int sock)
{
	...
	cnt = h.recmark - 4;
	PXY_FULL("xid %u is not on the list, skip %d bytes\n", h.xid, cnt);
	while (cnt > 0) {
			       ...
			       COMPOUND4args *args, COMPOUND4res *res)
{
	...
	if (xdr_callmsg(&x, &rmsg) && xdr_COMPOUND4args(&x, args)) {
		...
		do {
			...
			PXY_FULL("%ssend XID %u with %d bytes",
			   ...
			   nfs_resop4 *resoparray)
{
	...
	PXY_START_TIMER();
	__sync_fetch_and_add(&pxy_counters.nr_rpcs, 1);
	__sync_fetch_and_add(&pxy_counters.nr_inflight_rpcs, 1);
	...
	PXY_STOP_TIMER(rpc_time_ns);
	__sync_fetch_and_sub(&pxy_counters.nr_inflight_rpcs, 1);
 ...
 */
	...
	for (i = FSAL_PROXY_CONTEXT_COUNT; i > 0; i--) {
		struct pxy_rpc_io_context *c =
				...
				struct fsal_obj_handle **handle)
{
	...
	PXY_START_TIMER();
	__sync_fetch_and_add(&pxy_counters.nr_lookups, 1);
	st = pxy_lookup_impl(parent, op_ctx->fsal_export, op_ctx->creds, path,
			     handle);
	PXY_STOP_TIMER(lookup_time_ns);
...
#define FSAL_CLOSE_NB_OP_ALLOC 2
	...
	__sync_fetch_and_add(&pxy_counters.nr_closes, 1);
...
#define FSAL_CREATE_PLUS_NB_OP_ALLOC 5
	...
	PXY_START_TIMER();
	__sync_fetch_and_add(&pxy_counters.nr_createplus, 1);
	...
	*wrote_amount = wok->count;
	...
	/**
	 * The created file is still opened, to preserve the correct seqid for
	 * later use, we close it.
	 */
	st = pxy_do_close(op_ctx->creds, &fhok->object, &opok->stateid,
			  op_ctx->fsal_export);
	if (FSAL_IS_ERROR(st))
...
#define FSAL_MKDIR_NB_OP_ALLOC 4
	...
	__sync_fetch_and_add(&pxy_counters.nr_mkdirs, 1);
				 ...
				 fsal_readdir_cb cb, bool *eof)
{
	...
	__sync_fetch_and_add(&pxy_counters.nr_readdirs, 1);
...
#define FSAL_RENAME_NB_OP_ALLOC 4
	...
	__sync_fetch_and_add(&pxy_counters.nr_renames, 1);
...
static fsal_status_t pxy_getattrs(struct fsal_obj_handle *obj_hdl)
{
	...
	PXY_START_TIMER();
	__sync_fetch_and_add(&pxy_counters.nr_getattrs, 1);

	ph = container_of(obj_hdl, struct pxy_obj_handle, obj);
	st = pxy_getattrs_impl(op_ctx->creds, op_ctx->fsal_export,
			       &ph->fh4, &obj_attr);
	if (!FSAL_IS_ERROR(st)) {
		update_attributes(&obj_attr, &obj_hdl->attributes);
		set_attr_expire_time(&obj_hdl->attributes,
				     FSAL_PROXY_ATTR_EXPIRE_SECONDS);
	}
	PXY_STOP_TIMER(getattr_time_ns);
...
#define FSAL_SETATTR_NB_OP_ALLOC 3
	...
	__sync_fetch_and_add(&pxy_counters.nr_setattrs, 1);
	...
	if (rc != NFS4_OK) {
		LogWarn(COMPONENT_FSAL,
			...
			"ignoring attributes after making changes", rc);
	} else {
		PXY_DBG("file size set from %zd to %zd\n",
			obj_hdl->attributes.filesize, attrs_after.filesize);
...
#define FSAL_UNLINK_NB_OP_ALLOC 3
	...
	__sync_fetch_and_add(&pxy_counters.nr_unlinks, 1);
			      ...
			      fsal_openflags_t openflags)
{
	...
	__sync_fetch_and_add(&pxy_counters.nr_opens, 1);
	ph = container_of(obj_hdl, struct pxy_obj_handle, obj);
...
#define FSAL_READ_NB_OP_ALLOC 2
	...
	PXY_START_TIMER();
	ops_hist_add(&pxy_counters.ops_hist, offset, buffer_size);
	__sync_fetch_and_add(&pxy_counters.nr_reads, 1);
	...
	*read_amount = rok->data.data_len;
	PXY_DBG("read %zd bytes at offset %zd with end-of-file %s\n",
		*read_amount, offset, (*end_of_file ? "set" : "unset"));
	PXY_STOP_TIMER(read_time_ns);
...
#define FSAL_WRITE_NB_OP_ALLOC 2
	...
	PXY_START_TIMER();
	ops_hist_add(&pxy_counters.ops_hist, offset, size);
	__sync_fetch_and_add(&pxy_counters.nr_writes, 1);
			    ...
			    struct attrlist *attrs)
{
	...
	int opcnt = 0;
	struct pxy_obj_handle *ph;
#define FSAL_READ_PLUS_NB_OP_ALLOC 3
	...
	GETATTR4resok *atok;
	char fattr_blob[FATTR_BLOB_SZ];
	bool normal_read = (info->io_content.what == NFS4_CONTENT_DATA);
	READ4resok *rok;

	PXY_START_TIMER();
	ops_hist_add(&pxy_counters.ops_hist, offset, buffer_size);
	__sync_fetch_and_add(&pxy_counters.nr_readplus, 1);
	...
	if (attrs) {
		atok = pxy_fill_getattr_reply(resoparray + opcnt, fattr_blob,
					      sizeof(fattr_blob));
		COMPOUNDV4_ARG_ADD_OP_GETATTR(
		    opcnt, argoparray,
		    pxy_attrmask_to_bitmap(attrs->mask, false));
	}
	...
	if (attrs) {
		if (nfs4_Fattr_To_FSAL_attr(attrs, &atok->obj_attributes,
					    NULL) != NFS4_OK)
			return fsalstat(ERR_FSAL_INVAL, 0);
		set_attr_expire_time(attrs, FSAL_PROXY_ATTR_EXPIRE_SECONDS);
	}
	...
	PXY_STOP_TIMER(readplus_time_ns);
			     ...
			     struct attrlist *attrs)
{
	int rc;
	int opcnt = 0;
#define FSAL_WRITE_PLUS_NB_OP_ALLOC 3
	...
	PXY_START_TIMER();
	ops_hist_add(&pxy_counters.ops_hist, offset, size);
	__sync_fetch_and_add(&pxy_counters.nr_writeplus, 1);
	...
	if (normal_read) {
		...
		COMPOUNDV4_ARG_ADD_OP_WRITE_PLUS(opcnt, argoparray,
						 (&info->io_content));
	}
	...
	if (normal_read) {
		*write_amount = wok->count;
		*fsal_stable = wok->committed != UNSTABLE4;
	} else {
		*write_amount = wpr4->wr_count;
		*fsal_stable = wpr4->wr_committed != UNSTABLE4;
	}
	PXY_STOP_TIMER(writeplus_time_ns);
...
static fsal_status_t pxy_commit(struct fsal_obj_handle *obj_hdl,
				...
				size_t len)
{
	__sync_fetch_and_add(&pxy_counters.nr_commits, 1);
	return fsalstat(ERR_FSAL_NO_ERROR, 0);
}
