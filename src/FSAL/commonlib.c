...
void lock_and_append(const char *fname, const char *buf, size_t len)
{
	...
	fd = open(fname, O_CREAT | O_APPEND | O_RDWR, S_IRUSR | S_IWUSR);
	if (fd < 0) {
		error(errno, errno, fname);
	}
...
void update_attributes(const struct attrlist *new_attrs, struct attrlist *attrs)
{
	if (new_attrs->mask & ATTR_TYPE)
		attrs->type = new_attrs->type;
	if (new_attrs->mask & ATTR_SIZE)
		attrs->filesize = new_attrs->filesize;
	if (new_attrs->mask & ATTR_FSID)
		attrs->fsid = new_attrs->fsid;
	if (new_attrs->mask & ATTR_ACL)
		*attrs->acl = *new_attrs->acl;
	if (new_attrs->mask & ATTR_FILEID)
		attrs->fileid = new_attrs->fileid;
	if (new_attrs->mask & ATTR_MODE)
		attrs->mode = new_attrs->mode;
	if (new_attrs->mask & ATTR_NUMLINKS)
		attrs->numlinks = new_attrs->numlinks;
	if (new_attrs->mask & ATTR_OWNER)
		attrs->owner = new_attrs->owner;
	if (new_attrs->mask & ATTR_GROUP)
		attrs->group = new_attrs->group;
	if (new_attrs->mask & ATTR_RAWDEV)
		attrs->rawdev = new_attrs->rawdev;
	if (new_attrs->mask & ATTR_ATIME)
		attrs->atime = new_attrs->atime;
	if (new_attrs->mask & ATTR_CREATION)
		attrs->creation = new_attrs->creation;
	if (new_attrs->mask & ATTR_CTIME)
		attrs->ctime = new_attrs->ctime;
	if (new_attrs->mask & ATTR_MTIME)
		attrs->mtime = new_attrs->mtime;
	if (new_attrs->mask & ATTR_CHGTIME)
		attrs->chgtime = new_attrs->chgtime;
	if (new_attrs->mask & ATTR_SPACEUSED)
		attrs->spaceused = new_attrs->spaceused;
	if (new_attrs->mask & ATTR_CHANGE)
		attrs->change = new_attrs->change;
	if (new_attrs->mask & ATTR_EXPIRE_TIME_ATTR)
		attrs->expire_time_attr = new_attrs->expire_time_attr;

	attrs->mask |= new_attrs->mask;
}
