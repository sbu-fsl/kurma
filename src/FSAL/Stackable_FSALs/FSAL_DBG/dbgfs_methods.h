/* DBGFS methods for handles
 */

struct dbgfs_fsal_obj_handle;

struct dbgfs_file_handle {
	int nothing;
};
...
/*
 * DBGFS internal export
 */
struct dbgfs_fsal_export {
...
struct dbgfs_fsal_obj_handle {
	...
	struct dbgfs_file_handle *handle;
	union {
		struct {
			int fd;
		...
		struct {
			struct dbgfs_file_handle *dir;
			char *name;
		} unopenable;
	} u;
};

int dbgfs_fsal_open(struct dbgfs_fsal_obj_handle *, int, fsal_errors_t *);
int dbgfs_fsal_readlink(struct dbgfs_fsal_obj_handle *, fsal_errors_t *);

static inline bool dbgfs_unopenable_type(object_file_type_t type)
{
...
fsal_status_t dbgfs_share_op(struct fsal_obj_handle *obj_hdl, void *p_owner,
...
fsal_status_t dbgfs_list_ext_attrs(struct fsal_obj_handle *obj_hdl,
				    unsigned int cookie,
				    fsal_xattrent_t *xattrs_tab,
				    unsigned int xattrs_tabsize,
				    unsigned int *p_nb_returned,
				    int *end_of_list);
fsal_status_t dbgfs_getextattr_id_by_name(struct fsal_obj_handle *obj_hdl,
					   const char *xattr_name,
					   unsigned int *pxattr_id);
fsal_status_t dbgfs_getextattr_value_by_name(struct fsal_obj_handle *obj_hdl,
					      const char *xattr_name,
					      caddr_t buffer_addr,
					      size_t buffer_size,
					      size_t *p_output_size);
fsal_status_t dbgfs_getextattr_value_by_id(struct fsal_obj_handle *obj_hdl,
					    unsigned int xattr_id,
					    caddr_t buffer_addr,
					    size_t buffer_size,
					    size_t *p_output_size);
fsal_status_t dbgfs_setextattr_value(struct fsal_obj_handle *obj_hdl,
				      const char *xattr_name,
				      caddr_t buffer_addr, size_t buffer_size,
				      int create);
fsal_status_t dbgfs_setextattr_value_by_id(struct fsal_obj_handle *obj_hdl,
					    unsigned int xattr_id,
					    caddr_t buffer_addr,
					    size_t buffer_size);
fsal_status_t dbgfs_getextattr_attrs(struct fsal_obj_handle *obj_hdl,
				      unsigned int xattr_id,
				      struct attrlist *p_attrs);
fsal_status_t dbgfs_remove_extattr_by_id(struct fsal_obj_handle *obj_hdl,
					  unsigned int xattr_id);
fsal_status_t dbgfs_remove_extattr_by_name(struct fsal_obj_handle *obj_hdl,
					    const char *xattr_name);
