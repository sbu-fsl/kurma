...
/* helpers to/from other DBG objects
 */
...
{
	struct dbgfs_fsal_export *myself;
	struct fsal_module *sub_fsal;

	myself = container_of(exp_hdl, struct dbgfs_fsal_export, export);
...
/* dbgfs_export_ops_init
 * overwrite vector entries with the methods that we support
 */

void dbgfs_export_ops_init(struct export_ops *ops)
{
	ops->release = release;
	ops->lookup_path = dbgfs_lookup_path;
	ops->extract_handle = extract_handle;
	ops->create_handle = dbgfs_create_handle;
...
static struct config_item export_params[] = {
	...
	CONF_RELAX_BLOCK("FSAL", sub_fsal_params,
			 ...
			 dbgfsal_args, subfsal),
	CONFIG_EOL
};

static struct config_block export_param = {
	.dbus_interface_name = "org.ganesha.nfsd.config.fsal.dbgfs-export%d",
	.blk_desc.name = "FSAL",
				   ...
				   const struct fsal_up_vector *up_ops)
{
	...
	fsal_stack = lookup_fsal(dbgfsal.subfsal.name);
	if (fsal_stack == NULL) {
		LogMajor(COMPONENT_FSAL,
			 "dbgfs_create_export: failed to lookup for FSAL %s",
			 dbgfsal.subfsal.name);
		return fsalstat(ERR_FSAL_INVAL, EINVAL);
	}

	myself = gsh_calloc(1, sizeof(struct dbgfs_fsal_export));
	if (myself == NULL) {
		LogMajor(COMPONENT_FSAL,
	...
	dbgfs_export_ops_init(myself->export.ops);
	dbgfs_handle_ops_init(myself->export.obj_ops);
	...
	LogDebug(COMPONENT_FSAL, "fsal_export %p", &myself->export);
