 ...
 * Copyright 2016 Chuan Qin and Mingqiang Li
 * All rights reserved.
 *
...
CDCodec::CDCodec(int CDType, int n, int m, int r, CryptoPrimitive *cryptoObj) {
    ...
    if (cryptoObj_ == NULL) {
        DBG("Error: no CryptoPrimitive instance for hash generation and data encryption!\n");				
        exit(1);
    }

    if (CDType_ == CRSSS_TYPE) { /*CDCodec based on CRSSS*/
        if (n <= 0) {			
            DBG("Error: n should be > 0!\n");				
            exit(1);		
        }
        if ((m <= 0) || (m >= n)) {			
            DBG("Error: m should be in (0, n)!\n");				
            exit(1);		
        }
        if (n - m <= 1) {			
            DBG("Error: k = n -m should be > 1 for further providing confidentiality!\n");				
            exit(1);	
        }
        if ((r <= 0) || (r >= n - m)){			
            DBG("Error: r should be in (0, n-m)!\n");				
            exit(1);		
        }
        n_ = n;		
        m_ = m;		
        ...
        secretSizeAlignment_ =  bytesPerSecretWord_ * k_;
        ...
        if (!gf_init_easy(&gfObj_, bitsPerGFWord_)) {			
            DBG("Error: bad gf specification\n");			
            exit(1);		
        } 
        ...
        DBG("      k_: %d \n", k_);		
        DBG("      r_: %d \n", r_);			
        DBG("      bytesPerSecretWord_: %d \n", bytesPerSecretWord_);		
        DBG("      bitsPerGFWord_: %d \n", bitsPerGFWord_);				
        DBG("      distributionMatrix_: (see below) \n");		
        for (i = 0; i < n_; i++) {
            DBG("         | ");
            for (j = 0; j < k_; j++) {
                DBG("%3d ", distributionMatrix_[k_* i + j]);
            }
    ...
    if ((CDType_ == AONT_RS_TYPE) || (CDType_ == OLD_CAONT_RS_TYPE) || 
        ...
        if (cryptoObj_->getHashSize() == cryptoObj_->getKeySize()) {
            ...
            DBG("Error: the hash size is not equal to the key size in the input CryptoPrimitive instance!\n");				
            exit(1);		
        }
...
bool CDCodec::crsssEncoding(unsigned char *secretBuffer, int secretSize, unsigned char *shareBuffer, int *shareSize) {
    ...
    if (alignedSecretBufferSize_ < alignedSecretSize) {		
        DBG("Error: please use an internal alignedSecretBuffer_[] of size >= %d bytes!\n", alignedSecretSize);
    ...
    if (erasureCodingDataSize_ < (*shareSize) * k_) {		
        DBG("Error: please use an internal erasureCodingData_[] of size >= %d bytes!\n", (*shareSize) * k_);
    ...
    for (i = 0; i < numOfGroups; i++) {
        ...
        for (j = 0; j < r_; j++) {			
            ...
            if (!cryptoObj_->generateHash(hashInputBuffer_, bytesPerGroup_ + 1, rHashes_ + bytesPerSecretWord_ * j)) {
                DBG("Error: fail in the hash calculation!\n");
...
bool CDCodec::crsssDecoding(unsigned char * shareBuffer, int *kShareIDList, int shareSize, 
    ...
    if ((shareSize % bytesPerSecretWord_) != 0) {		
        DBG("Error: the share size (i.e. %d bytes) should be a multiple of secret word size (i.e. %d bytes)!\n", 
    ...
    if (secretSize > alignedSecretSize) {		
        DBG("Error: the input secret size (%d bytes) cannot exceed %d bytes!\n", secretSize, alignedSecretSize);
    ...
    if (!squareMatrixInverting()) {
        DBG("Error: a k * k submatrix of the distribution matrix is noninvertible!\n");
    ...
    for (i = 0; i < numOfGroups; i++) {
        ...
        for (j = 0; j < r_; j++) {
            ...
            if (memcmp(erasureCodingData_ + shareSize * (secretWordsPerGroup_ + j) + bytesPerSecretWord_ * i, 
                ...
                DBG("Error: fail in integrity checking!\n");
...
/*
 * derive the share size if encoding a secret of the given size.
 *
 * @param secretSize - the size of the secret
 *
 * @return - the share size
 */
int CDCodec::getShareSize(int secretSize) {
    int shareSize;
    ...
    if (CDType_ == CRSSS_TYPE) {
        ...
        /*align the secret size into alignedSecretSize*/
        alignedSecretSize = getAlignedSecretSize(secretSize);
        /*deduce the share size into shareSize*/
...
/*
 * get the size of aligned secret size.
 *
 * @param secretSize - the size of the secret
 *
 * @return - the minimum aligned secret size
 */
int CDCodec::getAlignedSecretSize(int secretSize) {
    int paddedSize = secretSize + bytesPerSecretWord_;  // for Hash
    int totalSizeofAllShares =
            ((paddedSize + secretSizeAlignment_ - 1) / secretSizeAlignment_) *
            secretSizeAlignment_;
    return totalSizeofAllShares - bytesPerSecretWord_;
}
 ...
 * @return - a boolean value that indicates if the encoding succeeds
 */
bool CDCodec::aontRSEncoding(unsigned char *alignedSecretBuffer,
                             int alignedSecretSize, unsigned char *shareBuffer,
                             int *shareSize) {
    int numOfSecretWords;
    int coef;
    int i, j;

    assert(alignedSecretSize == getAlignedSecretSize(alignedSecretSize));
    ...
    for (i = 0; i < numOfSecretWords; i++) {
        ...
        if (!cryptoObj_->encryptWithKey(wordForIndex_, bytesPerSecretWord_, key_, erasureCodingData_ + bytesPerSecretWord_ * i)) {
            DBG("Error: fail in the data encryption!\n");
        ...
        coef = 1;
        gfObj_.multiply_region.w32(&gfObj_, alignedSecretBuffer + bytesPerSecretWord_ * i,
 ...
 * @return - a boolean value that indicates if the encoding succeeds
 */
bool CDCodec::caontRSOldEncoding(unsigned char *alignedSecretBuffer,
    ...
    if (!cryptoObj_->generateHash(alignedSecretBuffer, alignedSecretSize, key_)) {
        DBG("Error: fail in the hash calculation!\n");
 ...
 * @return - a boolean value that indicates if the encoding succeeds
 */
bool CDCodec::caontRSEncoding(unsigned char *alignedSecretBuffer,
                              int alignedSecretSize, unsigned char *shareBuffer,
                              int *shareSize) {
    int coef;
    int i, j;
    ...
    coef = 1;
    gfObj_.multiply_region.w32(&gfObj_, alignedSecretBuffer, erasureCodingData_, coef, alignedSecretSize, 1);
...
bool CDCodec::caontRSDecoding(unsigned char * shareBuffer, int *kShareIDList, int shareSize, 
        int secretSize, unsigned char *secretBuffer) {
    unsigned char *alignedSecretBuffer;
    int alignedSecretSize;
    int coef;
    int i, j;	
    ...
    alignedSecretBuffer = (alignedSecretSize == secretSize) ? secretBuffer : alignedSecretBuffer_;
    ...
    if (!cryptoObj_->encryptWithKey(alignedSizeConstant_, alignedSecretSize, key_, alignedSecretBuffer)) {
        DBG("Error: fail in the data encryption!\n");
    ...
    coef = 1;
    gfObj_.multiply_region.w32(&gfObj_, erasureCodingData_, alignedSecretBuffer, coef, alignedSecretSize, 1);

    /*generate a hash from the aligned secret, and temporarily store it in the front end of erasureCodingData_*/
    if (!cryptoObj_->generateHash(alignedSecretBuffer, alignedSecretSize, erasureCodingData_)) {
        DBG("Error: fail in the hash calculation!\n");
    ...
    if (secretBuffer != alignedSecretBuffer) {
        memcpy(secretBuffer, alignedSecretBuffer, secretSize);
    }
 ...
 *
 * @param secretBuffer - a buffer that stores the secret
 * @param secretBufferSize - the size of the secret buffer.  It might
 *        be larger than the secretSize and leave some room for padding
 *        to avoid unnecessary memory copy.  Note that secretBufferSize
 *        bytes of the buffer may be changed.
 * @param secretSize - the size of the secret
 * @param shareBuffer - a buffer for storing the n generated shares <return>
 * @param shareBufferSize - the size of the share buffer.
 * @param shareSize - the size of each share <return>
 *
 * @return - a boolean value that indicates if the encoding succeeds
 */
bool CDCodec::encodingEx(unsigned char *secretBuffer, int secretBufferSize,
                         int secretSize, unsigned char *shareBuffer,
                         int shareBufferSize, int *shareSize) {
    bool success = false;

    if (CDType_ == CRSSS_TYPE) { /*CDCodec based on CRSSS*/
        return crsssEncoding(secretBuffer, secretSize, shareBuffer, shareSize);
    }

    int alignedSecretSize = getAlignedSecretSize(secretSize);
    if (alignedSecretBufferSize_ < alignedSecretSize) {
        DBG("Error: please use an internal alignedSecretBuffer_[] of size "
            ">= %d bytes!\n",
            alignedSecretSize);
        return false;
    }
    ...
    unsigned char *alignedSecretBuffer;
    if (secretBufferSize >= alignedSecretSize) {
        alignedSecretBuffer = secretBuffer;
    } else {
        memcpy(alignedSecretBuffer_, secretBuffer, secretSize);
        alignedSecretBuffer = alignedSecretBuffer_;
    }
    if (alignedSecretSize != secretSize) {
        memset(alignedSecretBuffer + secretSize, 0, alignedSecretSize - secretSize);
    }

    if (CDType_ == AONT_RS_TYPE) { /*CDCodec based on AONT-RS*/
        success = aontRSEncoding(alignedSecretBuffer, alignedSecretSize,
                                 shareBuffer, shareSize);
    }

    if (CDType_ == OLD_CAONT_RS_TYPE) { /*CDCodec based on old CAONT-RS*/
        success = caontRSOldEncoding(alignedSecretBuffer, alignedSecretSize,
                                     shareBuffer, shareSize);
    }

    if (CDType_ == CAONT_RS_TYPE) { /*CDCodec based on CAONT-RS*/
        success = caontRSEncoding(alignedSecretBuffer, alignedSecretSize,
                                  shareBuffer, shareSize);
    }
 ...
 * @return - a boolean value that indicates if the decoding succeeds
 */
bool CDCodec::decodingEx(unsigned char *shareBuffer, int shareBufferSize,
                         int *kShareIDList, int shareSize, int secretSize,
                         unsigned char *secretBuffer, int secretBufferSize) {
    bool success = 0;

    if (CDType_ == CRSSS_TYPE) { /*CDCodec based on CRSSS*/
        return crsssDecoding(shareBuffer, kShareIDList, shareSize, secretSize, secretBuffer);
    }

    if (CDType_ == AONT_RS_TYPE) { /*CDCodec based on AONT-RS*/
        success = aontRSDecoding(shareBuffer, kShareIDList, shareSize,
                                 secretSize, secretBuffer);
    }

    if (CDType_ == OLD_CAONT_RS_TYPE) { /*CDCodec based on old CAONT-RS*/
        success = caontRSOldDecoding(shareBuffer, kShareIDList, shareSize,
                                     secretSize, secretBuffer);
    }

    if (CDType_ == CAONT_RS_TYPE) { /*CDCodec based on CAONT-RS*/
        success = caontRSDecoding(shareBuffer, kShareIDList, shareSize,
                                  secretSize, secretBuffer);
    }
