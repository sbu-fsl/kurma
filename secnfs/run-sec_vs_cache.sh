#!/bin/bash -
# Launch the security proxy with caching and antivirus.
#
# This script should be placed in the build directory.
#
#       cp run-sec_vs_cache.sh <root-to-nfs-ganesha>/<build-directory>
#
...
#
#       <root-to-nfs-ganesha>/<build-directory>/run-sec_vs_cache.sh
...
if [[ ${running_mode} == 'debug' ]]; then
  CONFFILE=/etc/ganesha/sec_vs_cache.ganesha.conf
  ...
  CONFFILE=/etc/ganesha/sec_vs_cache.release.ganesha.conf
  LOGLEVEL='NIV_EVENT'
  export GLOG_v=0
  export GLOG_logtostderr=1
else
...
LOGFILE=/var/log/sec_vs_cache.ganesha.log
